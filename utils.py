...
def poisson_blend_old(input, output, mask):
    ...
    * returns:
    ...
    input_np = np.transpose(np.copy(input.cpu().numpy()), axes=(0, 2, 3, 1))   # (N, W, H, C)
    ...
    for i in range(num_samples):
        inpainted_np = blend(input_np[i], output_np[i], mask_np[i])  # (W, H, C)
        inpainted = torch.from_numpy(np.transpose(inpainted_np, axes=(2, 0, 1)))  # (C, W, H)
